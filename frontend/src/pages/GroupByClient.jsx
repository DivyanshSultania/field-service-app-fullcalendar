...
function getClientAssignmentRows(task) {
  const rows = [];
  const seenKeys = new Set();
  const clientName = task?.client_name || 'Unknown client';
  const clientKey = task?.client_id || clientName;
  ...
  const pushRow = (staffId, staffName) => {
    ...
    rows.push({
      ...task,
      grouped_client_key: clientKey,
      grouped_client_name: clientName,
...
function buildPdfHtml(groups, appliedFilters) {
  const pages = groups.map((group, index) => {
    ...
    const detailRows = group.rows.map(row => `
      <tr>
        <td>${escapeHtml(row.grouped_staff_name)}</td>
    ...
    return `
      <section class="page ${index < groups.length - 1 ? 'page-break' : ''}">
        ...
        <h3>${escapeHtml(group.clientName)}</h3>
        <div class="meta">
  ...
  return `
    ...
    <html lang="en">
      <head>
        <meta charset="utf-8" />
        <title>Client Schedule PDF</title>
        <style>
          body {
...
export default function GroupByClient({
  tasks,
  appliedFilters,
  showToast,
  loading,
  ...
  const clientRows = useMemo(() => {
    return tasks
      .flatMap(task => getClientAssignmentRows(task))
      .sort((firstRow, secondRow) => {
        const clientCompare = firstRow.grouped_client_name.localeCompare(secondRow.grouped_client_name);
        if (clientCompare !== 0) {
        ...
        return firstRow.grouped_staff_name.localeCompare(secondRow.grouped_staff_name);
      });
  }, [tasks]);
  ...
  const groups = useMemo(() => {
    ...
    clientRows.forEach(row => {
      const key = row.grouped_client_key || row.grouped_client_name;
      if (!groupedMap.has(key)) {
        groupedMap.set(key, {
          key,
          clientName: row.grouped_client_name,
          rows: [],
        });
      }
    ...
    return Array.from(groupedMap.values())
      ...
      .sort((firstGroup, secondGroup) => firstGroup.clientName.localeCompare(secondGroup.clientName));
  }, [clientRows]);
  ...
  function handleExportCsv() {
    if (!clientRows.length) {
      showToast?.('There are no client rows to export.', '#0f172a');
      return;
    }
    ...
    const lines = clientRows.map(row => [
      row.grouped_client_name,
    ...
    link.download = `schedule-group-by-client-${appliedFilters?.from || 'from'}-${appliedFilters?.to || 'to'}.csv`;
  ...
  function handlePrintPdf() {
    if (!groups.length) {
      showToast?.('There are no client groups to print.', '#0f172a');
      return;
    }
      ...
      >
        <div>
          ...
          <div style={{ marginTop: 8, fontSize: 14, fontWeight: 700, color: '#334155' }}>
            Clients : {groups.length} Staff Assignments : {clientRows.length}
          </div>
        </div>
                    ...
                    <tr style={{ borderTop: '1px solid #e5e7eb' }}>
                      <td style={bodyCellStyle}>
                        <button
                          ...
                          aria-label={open ? 'Collapse client section' : 'Expand client section'}
                        >
                          {open ? '▾' : '▸'}
                        </button>
                      </td>
                      <td style={{ ...bodyCellStyle, fontWeight: 600 }}>{group.clientName}</td>
                          ...
                          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            <thead>
                              <tr style={{ background: '#e5e7eb', color: '#475569' }}>
                                <th style={detailHeaderCellStyle}>Staff</th>
                                <th style={detailHeaderCellStyle}>Task</th>
                            ...
                            <tbody>
                              {group.rows.map(row => (
                                <tr key={`${group.key}-${row.id}-${row.grouped_staff_id}-${row.start_time || ''}`} style={{ borderTop: '1px solid #f1f5f9' }}>
                                  <td style={detailBodyCellStyle}>{row.grouped_staff_name}</td>
              ...
              {!groups.length && (
                <tr>
                  <td colSpan={8} style={{ padding: 28, textAlign: 'center', color: '#64748b' }}>
                    {loading ? 'Loading client report...' : 'No client rows found for the selected filters.'}
                  </td>
                </tr>
              )}
            </tbody>
